...
@section('title') Categorys @endsection
...
<!-- Shop Product Area -->
<div class="shop_page_area categories_page">
    <div class="container">
        <div class="shop_bar_tp fix">
            <form method="GET" id="filterForm">
                <div class="row">
                    <div class="col-sm-6 col-7 short_by_area">
                    ...
                    <div class="col-sm-6 col-5 show_area">
                        <div class="show_inner">
                            ...
                            <select class="show-select" name="show" onchange="document.getElementById('filterForm').submit()">
                                ...
                                <option value="{{ $categorys->total() }}" {{ request('show') == $categorys->total() ? 'selected' : '' }}>ALL</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-3 col-sm-6">
                    <div class="single_product" style="min-height: 250px !important;">
                        ...
                        <div class="product_btm_text">
                            <h4><a href="{{ route('categories.products', $category->slug) }}">{{ $category->name }}</a></h4>
                        </div>
                    </div>                                
                </div> 
                @endforeach
                ...
                <div class="pagination text-center">
                    @if ($categorys->hasPages())
                        <ul>
                            ...
                            @else
                                <li><a href="{{ $categorys->previousPageUrl() }}"><i class="fa fa-angle-left"></i></a></li>
                            @endif
        
                            {{-- Pagination Elements --}}
                            @foreach ($categorys->links()->elements as $element)
                                @if (is_array($element))
                                    @foreach ($element as $page => $url)
                                        <li>
                                            <a href="{{ $url }}" class="{{ ($page == $categorys->currentPage()) ? 'active' : '' }}">
                                                {{ $page }}
                                            </a>
                                        </li>
                                    @endforeach
                            ...
                            @if ($categorys->hasMorePages())
                                <li><a href="{{ $categorys->nextPageUrl() }}"><i class="fa fa-angle-right"></i></a></li>
                            @else
